...
export default function BoardPage({
  ...
  function handleClueClick(clue: Clue, categoryName: string) {
    ...
    if (clue.isDailyDouble) {
      const ddPlayer = game?.players.find((p) => p.id === currentPickerId) ?? game?.players[0];
      broadcast(id, {
        screen: "daily-double",
        categoryName,
        playerName: ddPlayer?.name,
        playerScore: ddPlayer?.score,
  ...
  return (
    <div className="flex flex-col h-screen no-select">
      ...
      <div className="shrink-0 text-center pt-2 pb-1">
        <p className="text-white/40 text-sm uppercase tracking-widest">{game.name}</p>
      </div>
      {/* Board */}
